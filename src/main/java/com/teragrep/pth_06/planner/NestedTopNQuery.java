...
public final class NestedTopNQuery {
    ...
    public Table<Record> getTableStatement(Condition journaldbConditionArg, Instant day) {
        ...
        SelectOnConditionStep<Record> selectOnConditionStep = DSL
                ...
                .innerJoin(
                        JOURNALDB.LOGFILE.forceIndex(Indexes.LOGFILE_CIX_LOGFILE_EPOCH_HOUR_HOST_ID_LOGTAG_ID.getName())
                )
        ...
        ULong start = ULong.valueOf(day.getEpochSecond());
        ULong end = ULong.valueOf(day.plusSeconds(86400).getEpochSecond());
        return selectOnConditionStep
                .where(JOURNALDB.LOGFILE.EPOCH_HOUR.greaterOrEqual(start).and(JOURNALDB.LOGFILE.EPOCH_HOUR.lessThan(end)).and(journaldbConditionArg)).orderBy(epochHourForOrderBy, JOURNALDB.LOGFILE.ID.asc()).asTable(innerTable);
    }
